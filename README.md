...
2.
    ...
    - Run `composer dev` for local develompent @ localhost:8000
...
- If you wish to access the PostreSQL DB in the container
run `psql -U lawnstarter_admin -d star_wars`

- For running the statistic computations programatically run:
...
command and looking at the `sail` setup and adapting it for the PostreSQL setup,
I tried to remove most of the uneeded boilerplate but some might've sneaked in there
